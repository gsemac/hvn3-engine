...
namespace hvn3 {
	...
	Font::Font(const std::string& filename, int size, FontFlags flags) {
		...
		_flags = flags;
		_font = font_shared_ptr_type(al_load_font(filename.c_str(), size, System::AllegroAdapter::ToFontFlags(flags)), al_destroy_font);
		_filename = filename;
		_size = size;
	...
	Font::Font(uint8_t* buffer, size_t buffer_size, int font_size, FontFormat format) :
		...
		case FontFormat::TrueTypeFont:
			_font = font_shared_ptr_type(al_load_ttf_font_f(file, "", font_size, NULL), al_destroy_font);
			break;
	...
	Font Font::BuiltIn() {
		...
		font._font = font_shared_ptr_type(al_create_builtin_font(), al_destroy_font);
		font._size = 8.0f;
	...
	void Font::AdjustScale(float scale_factor) {
		...
		if (scaled_font != nullptr)
			_font = font_shared_ptr_type(scaled_font, al_destroy_font);
